...
void saveReaderToFile(READER *listReader, int totalReader){
	...
	file.write(reinterpret_cast<char *> (&listReader),sizeof(READER)*(totalReader+1));
	cout<<"\n               Luu Doc Gia thanh cong!!!";
	file.close();
}
void readReaderFromFile(READER *listReader, int totalReader){
	ifstream file("READERS.bin", ios::binary);
	file.read(reinterpret_cast<char *> (&listReader),sizeof(READER)*(totalReader+1));
	file.close();
}
...
int main(){
	...
	READER *readers = new READER[1000];								//Khoi tao mang 1000 doc gia
	BORROW_RETURN *borrowReturnRecords = new BORROW_RETURN[4000];	//Khoi tao So ghi Phieu muon tra, 10000 record
	BOOK *books = new BOOK[5000];									//Khoi tao mang 10000 ddaauf sachs, sao cho nay bi tran Data :(
