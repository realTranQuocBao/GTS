// TQB13 - Tran Quoc Bao
// Da lien ket phieu muon/tra co tham chieu toi so sach remai
// kiem tra nhap cho 2.1, 1.1
#include <iostream>
#include <cstring>
#include <fstream>
#include <cstdlib>
...
READER enterAReader(READER *listReader, int totalReader){
	READER aReader;
	int k = 0, check=0;
	do{
...
void enterListReader(READER *listReader, int &totalReader){
	...
	for(int i=totalReader; i<totalReader+n; i++){
		...
		listReader[i] = enterAReader(listReader, totalReader);
		cout<<"Thanh Cong!!!\n";
	}
	totalReader += n;
}
